...
#         print('coco ind to class ind', self._coco_ind_to_class_ind)
        # *** e.g.)
        ...
        self.num_joints = 294
        self.gt_class_keypoints_dict = {1: (0, 4), 2: (4, 58), 3: (58, 89),
    ...
    def _load_coco_keypoint_annotation_kernal(self, index):
        ...
        :return: db entry
        """
        im_ann = self.coco.loadImgs(
            index)[0]  # <= [a_ for a_ in coco_json['images] if a_['id]==index][0]
        width = im_ann['width']
        height = im_ann['height']

        annIds = self.coco.getAnnIds(imgIds=index, iscrowd=False)
        # <= [a_ for a_ in coco_json['annotations'] if a_['image_id]==index]
        ...
        for obj in objs:
            ...
            joints_3d_vis = np.zeros(
                (self.num_joints, 3), dtype=float)  # np.float)
    ...
    def _load_deepfashion2_detection_results(self):
        ...
        if bbox_file_type == 'json':
            #             print('self.bbox_file : ',self.bbox_file)
    ...
    def evaluate(self, cfg, preds, output_dir, all_boxes, img_path,
        ...
        for idx, kpt in enumerate(preds):  # preds (2, 294, 3)
            _kpts.append({
                ...
                # *** acceptable only for last 6-digit
        ...
        for img in kpts.keys():  # each image
            ...
            if self.soft_nms:  # False
                keep = soft_oks_nms(
                    [img_kpts[i]
                        for i in range(len(img_kpts))],  # iter by each item
                    oks_thre
                )
            else:
                keep = oks_nms(
    ...
    def _write_coco_keypoint_results_DeepFashion2(self, keypoints, res_file):
        results = self._coco_keypoint_results_all_category_kernel(
            keypoints)  # *** Converting HERE!
    ...
    def _do_python_keypoint_eval(self, res_file, res_folder):
        ...
        coco_eval.params.kpt_oks_sigmas = np.array([0.012, 0.0158, 0.0169, 0.0165, 0.0169, 0.0158, 0.0298, 0.0329,
    ...
    def get_channel_index(self, class_id):
        gt_class_keypoints_dict = {
            1: (0, 4), 2: (4, 58), 3: (58, 89), 4: (89, 128), 5: (128, 143),
