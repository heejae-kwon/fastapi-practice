...
def _drawText(img, df: pd.DataFrame, id: int, line_k):
    cv2.putText(img, '{0}cm'.format(round(df['height'].loc[id]/10, 2)), (round(
        (line_k[0]+line_k[4])/2), round((line_k[1]+line_k[5])/2)), cv2.FONT_HERSHEY_SIMPLEX, 1.3, (0, 255, 0), 2)
    cv2.putText(img, '{0}cm'.format(round(df['width'].loc[id]/10, 2)),
                (line_k[2], line_k[3]), cv2.FONT_HERSHEY_SIMPLEX, 1.3, (0, 255, 0), 2)
    return (img)


def _getImageRatio(image: Image.Image, ply: PlyData):
    # Image size
    w, h = image.size

    # ply cx, cy
    # vert_ = ply['vertex']
    cx_w = float(ply['vertex']['cx'].max()) + 1    # 256.0
    cy_h = float(ply['vertex']['cy'].max()) + 1    # 192.0
...
def _get_proj_depth(ply: PlyData):
    vert_ = ply['vertex']
...
def kpt_visualization(temp_dir: Path):
    output_est = temp_dir / 'results/keypoints_validation_results_0.json'

    valid_dir = Path('strawberry/data/validation')
    valid_json_file = valid_dir / 'val-coco_style.json'
    # output = os.path.join(valid_dir, 'keypoints_validation_results_0.json')

    img_path = valid_dir / 'image'
    ply_path = valid_dir / 'ply'
    ...
    for id_ in range(len(img_values_list)):
        try:
            image = Image.open(img_path / img_values_list[id_])
            ply = PlyData.read(ply_path / ply_values_list[id_])
            w_r, h_r = _getImageRatio(image, ply)
    ...
    #         print(id_, img_values_list[id_])
            ...
            distance_a_to_c = np.sqrt(
                np.square(a[0]-c[0])+np.square(a[1]-c[1]))
            ...
            # b-c 점사이 거리 계산, width
            distance_b_to_d = np.sqrt(
                np.square(b[0]-d[0])+np.square(b[1]-d[1]))
    ...
    # plt.imshow(img)
    final_img_path = str(temp_dir/'final.png')
