...
def remove_background(input_path, model, alpha_matting=False):
...
"""
        ...
        pred = d1[:, 0, :, :]
        predict: torch.Tensor = norm_pred(pred)
...
async def run_rembg(image_path: Path):
    ...
    for i in range(len(real_imgs)):
        ...
        pred = remove_background(real_imgs[i], model, alpha_matting=False)

        out_file_path = Path(
            f"pred_{real_imgs[i].split('/')[-1].split('.')[0]}.png")
        pred.save(out_file_path)
        print('---------------------------------------------------')
